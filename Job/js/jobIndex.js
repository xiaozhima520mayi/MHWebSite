...
$(".inte_roundbg .mm").mouseover(function () {
    ...
    //    $(Thismsg).hide();
    //})
    //$(".msg").eq(indexs).addClass("fadeInLeftBig").removeClass("fadeInRightBig").siblings(".msg").removeClass("fadeInLeftBig").removeClass("fadeInRightBig");
}).mouseout(function () {
    auto = setTimeout(function () {
        $(".msg").hide();
    }, 1000)
...
function getMarg() {
    ...
    //第一屏
    var oh = ((h - 405) / 2) + ((h - 405) / 2) * 0.3;
    $(".sykj_content").css("margin-top", oh + "px");

    //第二屏
    //var th = ((h - 490) / 2) + ((h - 490) / 2);
    var th = ((h - 490) / 2) + 10;
    $(".camp_content").css("margin-top", th + "px");
    //第三屏
    var sh = ((h - 502) / 2) + ((h - 502) / 2) * 0.3;
    $(".inte_content").css("margin-top", sh + "px");
    //第四屏
    var wh = ((h - 484) / 2) + ((h - 484) / 2) * 0.3;
    $(".society_content").css("margin-top", wh + "px");
}
$(".nav").width($(window).width());
var f = new pageIndex($(".page>div"), 4, function (i) {
    $(".page>div").removeClass("e");
    $(".page>div").eq(i).addClass("e");
    if (i == 0) {
        $(".nav-sykj-right span").removeClass().addClass("nav-sykj-right_bg");
    } else if (i == 1) {
        $(".nav-sykj-right span").removeClass().addClass("nav-sykj-right_bg_camp");
    } else if (i == 2) {
        $(".nav-sykj-right span").removeClass().addClass("nav-sykj-right_bg_inte");
    } else if (i == 3) {
        $(".nav-sykj-right span").removeClass().addClass("nav-sykj-right_bg_society");
    }
    $(".nav-sykj>li").removeClass("e").eq(i).addClass("e");
    currentNav = i;
    FunLeft(currentNav);
});
var Tweens = { Linear: function (t, b, c, d) { return c * t / d + b; } }
$(".sykj_content_rb").mouseover(function (e) {
    clearTimeout(this.time);
    var o = $(this).find(".rb_circle_e");
    var of = $(this).find(".iconfonts");
    if (o.width() > 0) return;
    var top = 130, left = 130;
    var t = 0, d = 30;
    var o = $(this).find(".rb_circle_e");
    function _run() {
        if (t < d) {
            t++;
            var w = Math.ceil(Tween.Linear(t, 0, 132, d));
            o.width(w).height(w).css({ top: ((132 - w) / 2) + "px", left: ((132 - w) / 2) + "px" });
            if (t == 20) {
                opacityInfo(of, 0, 100);
            }
            setTimeout(_run, 10);
        } else {
            o.attr("s", "放大完成");
    ...
    this.time = setTimeout(function () {
        var top = 0, left = 0;
        var t = 0, d = 30;
        var o = ob.find(".rb_circle_e");
        var of = ob.find(".iconfonts");
        function _run() {
            if (t < d) {
                t++;
                var w = Math.ceil(Tween.Linear(t, 132, -132, d));
                o.width(w).height(w).css({ top: ((132 - w) / 2) + "px", left: ((132 - w) / 2) + "px" });
                if (t == 10) {
                    opacityInfo(of, 100, -100);
                }
                setTimeout(_run, 10);
            } else {
                o.attr("s", "缩小完成");
...
//培训发展处
//$("#content").niceScroll({ styler: "fb", cursorcolor: "#000" });
...
$(function () {
    ...
    $(".shouqi").click(function () {
        ...
        //$(".people_boxs .people_intro").removeClass("fadeInRightBig2"); 
        setTimeout(function () {
            $(".people_intro").removeClass("fadeInRightBig2");
            $(indexs).parent(".people_intro").addClass("hide");
        }, 200);
        //$(this).parent("div").addClass("fadeOutRightBig");
    ...
    if (Tops2 > 30) {
        $(".jianli").find(".dialog").css("margin-top", Tops2);
    }
    else {
        $(".jianli").find(".dialog").css("margin-top", "30px");
    }
    ...
    $(".society_group_itembg").click(function () {
        ...
        if (securrent == index) return;// alert(securrent+","+index)
        //$(this).parent().parent().find(".society_group_item:eq(" + securrent + ")").find(".society_group_itemcontent").animate({ height: 0, "padding-top": 0 }); 
...
function getModeltop(modal) {
    //弹出层高度
    modal.find(".dialog").each(function (i) {
        var modal = $(this);
        var modalHeight = $(modal).height();
        var screenHeight = $(window).height();
        var Tops = (screenHeight - modalHeight) / 2;
        if (Tops > 10) {
            $(modal).css("margin-top", Tops);
        }
        else {
...
window.onresize = function () {
    ...
    $(".screen").height(h);
    getMarg();
    var index = parseInt($(".page").children(".e").attr("index"));
    $(window).scrollTop(h * index);
}